...
* Cursor states are stored in the `bluesky-cursors` index and collected posts are stored in the `bluesky-posts` index.
...
* Timer-based crawling runs automatically every 2 minutes.
* Data is continuously written into the `bluesky-posts` index.
...
kubectl create secret generic es-secret \
  ...
  --from-literal=INDEX_NAME='bluesky-posts'
```
...
- This resets the crawler progress only. Existing post data in bluesky-posts will remain.
- Use this when you changed the code and want the crawler to re-run history crawling.

ii. Full reset from scratch
```{bash}
curl -k -u elastic:elastic -X DELETE https://localhost:9200/bluesky-posts
curl -k -u elastic:elastic -X DELETE https://localhost:9200/bluesky-cursors
```
...
```{bash}
curl -k -u elastic:elastic "https://localhost:9200/bluesky-posts/_count"
```
Expected output: {"count":xxx,"_shards":{"total":x,"successful":x,"skipped":x,"failed":x}}.

Meaning: The total number of data that have been collected and are being written to ES.
...
Package stuck in running → caused by heavy dependency (pandas)

Package failed → caused by Python version mismatch

Function timeout → caused by large batch crawling

Solutions:Removed pandas; Switched to Python 3.9; Implemented incremental crawling with small batches; These improvements made the system stable and compatible with Fission.
